...
namespace AcSaveConverter.Graphics
{
    internal class DdsLoader
    {
        ...
        private Texture LoadAsImageSharp(Memory<byte> bytes)
        {
            var simage = TextureConverter.LoadBcImage(bytes.Span);
            var vsimage = new ImageSharpTexture<Rgba32>(simage, false);
